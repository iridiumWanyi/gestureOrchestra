"""
Violin-ensemble instrument: keyboard pitch + hand-gesture volume.

A soft violin-section tone plays through your speakers:
    PITCH  : keyboard -- Q..P then 1..0 step up the white-key scale (G3..E6)
    VOLUME : right-hand openness -- fist = silent, spread = full

Run:
    ./venv/bin/python violin_ensemble.py
Press ESC in the video window to quit. Press 'c' to (re)calibrate the
...
MAX_AMPLITUDE = 0.35     # overall loudness ceiling at 100% volume (soft)
PITCH_MIN_HZ = 55.0      # keyboard pitch control clamps to this range
PITCH_MAX_HZ = 2000.0
SEMITONE = 2.0 ** (1.0 / 12.0)   # one-semitone frequency multiplier

# ---- Timbre: soft violin ensemble ---------------------------------------
# A gentle string-section sound, not a full orchestra. It's a pool of violins
# all playing the same note: each voice has its OWN random detuning, vibrato
# rate/phase and harmonic phases, so they bloom into a warm, breathing
# ensemble instead of one mechanical tone. A few voices double the octave up
# for sheen. Voices are panned across stereo for width. Everything renders
# live, so pitch (keyboard) and volume (hand) stay instant.
N_HARMONICS = 16
...
# Bigger rolloff = brighter; lower octave = deeper. Lower rolloff here keeps
# the violins warm and soft rather than edgy.
SECTIONS = [
    (1.0,  6.0, 10, 0.70),   # violin section (unison) -> warm ensemble body
    (2.0,  5.5,  4, 0.22),   # a few on the octave up  -> gentle sheen
]
...
VIB_RATE_RANGE = (5.0, 7.0)                 # violin-like vibrato speed (Hz)
VIB_DEPTH_RANGE = (0.004, 0.008)            # expressive violin vibrato depth
...
# Two key rows step UP the white-key (natural) scale, no sharps. The letter
# row Q..P is the lower octaves, the number row 1..0 continues above it, so
# the whole thing is one rising scale from G3 (Q) to E6 (0):
#
#   keys:  Q  W  E  R  T  Y  U  I  O  P  | 1  2  3  4  5  6  7  8  9  0
#   notes: G3 A3 B3 C4 D4 E4 F4 G4 A4 B4 | C5 D5 E5 F5 G5 A5 B5 C6 D6 E6
KEY_TO_NOTE = {
...
def open_camera():
    """Open a webcam that actually delivers non-black frames.

    On macOS the first index is often a Continuity Camera (iPhone) or a camera
    still warming up, which hands back all-black frames -- the usual cause of a
    black window. So we warm up each index and only accept one whose frames
    have real image content (std dev well above zero).
    """
    for index in (0, 1, 2):
        ...
        if not cap.isOpened():
            ...
            continue
        good = False
        for _ in range(30):                       # ~1s warm-up
            ok, frame = cap.read()
            if ok and frame is not None and float(frame.std()) > 5.0:
                good = True
                break
        if good:
            print(f"Using camera index {index}.")
            return cap
        print(f"Camera index {index} opened but returned only black frames; skipping.")
        cap.release()
    raise RuntimeError(
        "No camera delivered a usable (non-black) image. Common macOS causes:\n"
        " - A Continuity Camera (iPhone) grabbed the camera. Disable it in\n"
        "   System Settings > General > AirPlay & Handoff > Continuity Camera,\n"
        "   or keep the iPhone awake and mounted.\n"
...
def main():
    ...
    with TonePlayer() as tone, mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            cv2.putText(frame, "ESC quit   c calibrate   Q..P then 1..0 = notes G3..E6", (20, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (200, 200, 200), 1, cv2.LINE_AA)

            cv2.imshow("Violin Ensemble - ESC quit, c calibrate", frame)
